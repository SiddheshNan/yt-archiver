...
- Copies assets to backend runtime/frontend-build
- Prepares config
...
def copy_assets():
    ...
    dest_build = BACKEND_DIR / "runtime" / "frontend-build"
...
def prepare_docker_config():
    ...
    print("\n⚙️ Preparing docker configuration...")
    dev_config_path = RUNTIME_DIR / "config" / "dev.yaml"
    prod_config_path = RUNTIME_DIR / "config" / "prod.yaml"

    if not dev_config_path.exists():
        print(f"✗ Error: {dev_config_path} not found. Please create it first.")
        sys.exit(1)

    with open(dev_config_path, "r") as f:
        config = yaml.safe_load(f)

    # Modify MongoDB URL to target the host machine from inside Docker
    if "database" in config and "url" in config["database"]:
        config["database"]["url"] = config["database"]["url"].replace(
            "localhost", "host.docker.internal"
        ).replace("127.0.0.1", "host.docker.internal")

    # Modify tools to point to the Linux binaries inside the container
    if "tools" not in config:
        config["tools"] = {}
    config["tools"]["ytdlp_path"] = "yt-dlp"
    config["tools"]["ffmpeg_path"] = "ffmpeg"
    
    # Enable serving frontend
    if "server" not in config:
        config["server"] = {}
    config["server"]["serve_frontend"] = True
    config["server"]["frontend_build_dir"] = "runtime/frontend-build"

    prod_config_path.parent.mkdir(parents=True, exist_ok=True)
    with open(prod_config_path, "w") as f:
        yaml.dump(config, f)
    print(f"  ✓ Successfully generated {prod_config_path}")
